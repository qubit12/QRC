...
def main(
    ...
    # A = vshape_spin_large_3.get_system_under_study(num_qubits)
    ...
    bounds = Bounds(
        [0, 0, 0, -2, -2, -2 * np.pi] * number_of_pulses,  # type: ignore
        [2, 2, 2, 2, 2, 2 * np.pi] * number_of_pulses,  # type: ignore
    )
    parameter_start = np.array([1.2, 0.7, 1.5, 1.1, 1.8, 2] * number_of_pulses)
    ...
    def optimize_function(parameters):
        (
            ...
            onsite_parameters,
            times
        ) = vshape_spin_large_3.get_parameters_by_pulse(parameters, number_of_pulses)
        ...
        return np.mean(
            [
                reservoir.maths.infidelity(
                    vshape_spin_large_3.trace_out_reservoir(u @ initial_dm @ u.conj().T),
                    target_dm,
                )
                # np.linalg.norm(vshape_spin_large_3.trace_out_reservoir(u @ initial_dm @ u.conj().T) - target_dm, ord = 'fro')
                for u in unitaries
    ...
    save_output = {
        ...
        "operation": operation,
        "coupling_reservoirs": coupling_reservoirs,
    ...
    filename = f"{operation_name}|{num_qubits}|{number_of_pulses}|{seed}|{number_of_reservoirs}" + "_spin_large_3"
